...
class ShowOrders extends StatefulWidget {
  ...
  const ShowOrders({Key? key, required this.userId, 
  required this.orderType,
   }) : super(key: key);

  @override
  State<ShowOrders> createState() => _OrderPageState();
}

class _OrderPageState extends State<ShowOrders> {
  List<dynamic> orders = [];
...
Future<void> fetchOrders() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        orders = responseData['merchantOrders'].where((order) => order['status'] == widget.orderType).toList();
